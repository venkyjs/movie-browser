...
module.exports = {
	...
	getRips: lang => {
		...
		for (const eachSection of SECTIONS) {
			if (
				eachSection.language === lang &&
				rips.indexOf(eachSection.rip) < 0
			) {
...
const isPinned = currentNode => {
	if (
		...
		currentNode.parent.parent.parent.children[1].children[0].data ===
			'Pinned'
	) {
		return true
	} else {
		...
		.then(response => {
			...
			for (let i = 0; i < names.length; i++) {
				...
				if (linkId && !data[name][linkId]) {
					...
					if (brkIdx >= 0) {
						movieName =
							brkIdx >= 0 ? linkText.substr(0, brkIdx) : linkText
